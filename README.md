...
本文の読者別の役割は変えない。「家庭での準備」は軟化の補助ページとして案内し、その後の制作は初級の道具確認へ接続する。
質問欄の呼び方は「気になること」に統一し、既存の beginner#questions へ接続する。開催日時など未提供の情報は作らない。
...
初級の 5 工程と公開済みのアンカーは維持する。目次には制作後の「作った標本を楽しもう」も載せる。
「もっとくわしく」は作る目的・配置と見せ方・観察と比較・次の制作の 4 章。
...
「家庭での準備」は軟化の補助ページとして、準備・方法選択・方法 A・方法 B・状態確認・作業再開の目次にする。
全体の 5 工程の目次を重ねない。方法 A/B は選択肢として示し、どちらの終了リンクも共通の確認へ接続する。
方法の本文は初期状態で閉じた details に入れ、選んだ一方を開いて読む。JavaScript 無効でも見出しから開閉できる。
方法へのリンク・検索結果・既存のブックマークでは必要な details を開く。同じハッシュのリンクを再選択した場合も開く。
2 つの写真枠は、それぞれの方法の冒頭に置く。条件・時間の目安・確認方法は本文に残す。
各ガイドの問い合わせ案内は participant-contact トークンで最後のページ移動の直前に置く。
問い合わせリンクの受付状態は src/site.mjs にまとめ、メニュー・フッター・本文の案内で共通表示する。
